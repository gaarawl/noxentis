...
export function ActivityTimeline({ items }: { items: ActivityItem[] }) {
  return (
    <div className="space-y-4">
      {items.map((item) => (
        <div key={item.id} className="flex items-start gap-4 rounded-2xl border border-white/6 bg-white/[0.03] p-4">
          ...
          <p className="text-xs uppercase tracking-[0.18em] text-white/35">{formatDate(item.timestamp)}</p>
        </div>
      ))}
    </div>
  );
